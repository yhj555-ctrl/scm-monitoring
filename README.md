...
**6개 메인 페이지(`/`, `/suppliers`, `/materials`, `/news`, `/logistics`, `/competitors`)** 상단에
그날의 핵심 데이터를 3~4줄로 요약하는 파란색 박스("📋 금일 주요내용 요약")가 있습니다. 각 페이지가
이미 조회한 데이터(리스크 건수, 최대 변동 품목, 최근 기사, 지연 구간 등)를 그 자리에서 계산해
보여주는 것이라 별도 API 호출은 없습니다. 구현: `components/SummaryPanel.tsx` — `lines: string[]`만
받는 순수 서버 컴포넌트라 항상 안전합니다(함수를 props로 넘기지 않음).

## 상단 지표 카드 클릭 → 세부내용 팝업

`/`(리스크 대시보드)와 `/suppliers`(업체평가 리스크 관리) 상단의 숫자 카드(예: "총 모니터링 항목",
"리스크 [상] 발생", "관리 대상 공급업체", "평균 종합점수")를 클릭하면 그 숫자를 구성하는 개별 항목을
팝업으로 리스트업해서 보여줍니다. 구현: `components/ClickableMetricCard.tsx` — 클릭 시 여닫는
모달에 `{primary, secondary, tag}` 형태의 목록만 props로 받는 클라이언트 컴포넌트입니다.
